...
public class CusReplyDAO {
	...
	public int inserComend(replyBean bean,int CL_ID) {
		...
		String profile = null;
		try {
			 sql = "select cus_profile_path from customer where cus_id=?";
			 pstmt=con.prepareStatement(sql);
			 pstmt.setString(1, bean.getComment_id());
			  rs = pstmt.executeQuery();
			 while(rs.next()) {
				 profile =(rs.getString("cus_profile_path"));
		 ...
		 sql = "insert into class_qna(CLQA_CLASS,CLQA_WRITER_ID,CLQA_CONTENT,CLQA_REGDATE,cus_profile_path) values(?,?,?,now(),?)";
		try {
			pstmt = con.prepareStatement(sql);
	...
	try {
		String sql = "select CLQA_num, CLQA_CLASS ,CLQA_WRITER_ID , CLQA_CONTENT ,CLQA_REGDATE, cus_profile_path from class_qna where CLQA_CLASS =? order by CLQA_NUM desc";
		pstmt = con.prepareStatement(sql);
		pstmt.setInt(1, CL_ID);
		rs=pstmt.executeQuery();
		while(rs.next()) {
	...
	public int insertRecomment(recomment_bean bean) {
		...
		try {
			 sql = "select cus_profile_path from customer where cus_id=?";
			 pstmt=con.prepareStatement(sql);
			 pstmt.setString(1,bean.getRecomment_id());
			  rs = pstmt.executeQuery();
			 while(rs.next()) {
				 profile =(rs.getString("cus_profile_path"));
		 ...
		 sql= "insert into recomment_qna(recomment_comment_num,recomment_content,recomment_id,recomment_date,cus_profile_path) values(?,?,?,now(),?)";
		try {
			pstmt = con.prepareStatement(sql);
	...
	public ArrayList<recomment_bean> getCommentlist() {
		...
		ResultSet rs = null;
		 ...
		 String sql = "select recomment_num,recomment_comment_num,recomment_content,recomment_id,recomment_date, cus_profile_path from recomment_qna order by recomment_num desc";
		 try {
			 pstmt = con.prepareStatement(sql);
			 rs=pstmt.executeQuery();
			 while(rs.next()) {
