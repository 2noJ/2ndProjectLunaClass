...
public class MyParticipateListEndService {
	...
	public ArrayList<ClassBean> getParticipateList(User authUser, int page, int limit) throws Exception {
		...
		CustomerDAO customerDAO = CustomerDAO.getInstance();
		customerDAO.setConnection(con);
		articleList = customerDAO.selectMyParticipateListEnd(authUser, page, limit);
		close(con);
		return articleList;
