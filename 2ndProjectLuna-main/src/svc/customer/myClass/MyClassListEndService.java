...
public class MyClassListEndService {
	...
	public ArrayList<ClassBean> getClassList(User authUser, int page, int limit) throws Exception {
		...
		CustomerDAO customerDAO = CustomerDAO.getInstance();
		customerDAO.setConnection(con);
		articleList = customerDAO.selectMyClassListEnd(authUser, page, limit);
		close(con);
		return articleList;
