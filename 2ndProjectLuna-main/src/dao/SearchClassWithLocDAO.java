...
public class SearchClassWithLocDAO {
	DataSource ds;
	Connection con;

	private static SearchClassWithLocDAO customerDAO;

	private SearchClassWithLocDAO() {
	}

	public static SearchClassWithLocDAO getInstance() {
		if (customerDAO == null) {
			customerDAO = new SearchClassWithLocDAO();
		}
		return customerDAO;
	}
	...
	public int selectSearchClassWithLocListCount(String CL_NAME, String CL_LOCATION) {
		int listCount = 0;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
				pstmt = con.prepareStatement("SELECT count(*) FROM CLASS where (CL_NAME like ? or CL_CONTENT like ?) and CL_LOCATION = ?");
	...
	public ArrayList<ClassBean> selectSearchClassWithLocList(String CL_NAME, String CL_LOCATION, int page, int limit) {

		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql2 = "select * from CLASS where (CL_NAME like ? or CL_CONTENT like ?) and CL_LOCATION = ? order by CL_ID desc limit ?,6";
